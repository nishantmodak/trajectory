...
def render_transcript(data: SessionData) -> str:
    """Render full conversation transcript as markdown.

    Formats the complete session as a readable conversation with:
    - User and assistant messages
    - Tool calls summarized as one-liners
    ...
    if data.project_path:
        lines.append(f"**Project:** `{data.project_path}`")
    if data.git_branch:
        ...
        if turn.role == "user":
            ...
            if turn.tool_calls:
                for tc in turn.tool_calls:
                    tool_line = _format_tool_call(tc, data.project_path)
                    if tool_line:
    ...
    # Footer
    lines.append(f"_Generated by [trajectory](https://github.com/nishantmodak/trajectory-ai)_")

    return "\n".join(lines)


def _format_tool_call(tc, project_path: str) -> str:
    """Format a tool call as a one-liner summary."""
    name = tc.name
    inp = tc.input

    if name == "Read":
        path = _relativize_path(inp.get("file_path", ""), project_path)
        return f"`[Read]` {path}"
    ...
    elif name in ("TodoWrite", "AskUserQuestion"):
